<template>
  <div class="hls-connections">
    <el-card>
      ...
      <el-table :data="hlsConnStore.list" style="width: 100%" v-loading="hlsConnStore.loading">
...
import { useHlsConnStore } from '@/stores/hlsConn'

const hlsConnStore = useHlsConnStore()
...
const showDetails = async (connection: any) => {
  try {
    currentConnection.value = await hlsConnStore.getConnection(connection.id)
...
const handleClose = async (connection: any) => {
  try {
    ...
    await hlsConnStore.closeConnection(connection.id)
...
const handleCloseAll = async () => {
  try {
    ...
    await hlsConnStore.closeAllConnections()
