<template>
  <div class="rtmp-connections">
    <el-card>
      ...
      <el-table :data="rtmpConnStore.list" style="width: 100%" v-loading="rtmpConnStore.loading">
...
const rtmpConnStore = useRtmpConnStore()
...
const showDetails = async (connection: any) => {
  try {
    currentConnection.value = await rtmpConnStore.getConnection(connection.id)
...
const handleClose = async (connection: any) => {
  try {
    ...
    await rtmpConnStore.closeConnection(connection.id)
...
const handleCloseAll = async () => {
  try {
    ...
    await rtmpConnStore.closeAllConnections()
